...
import { TvAiringTodayApi } from '../../../apis';
import { Col, Container, Row } from 'react-bootstrap';
import SubSlider from './layout/SubSlider';

const TVAiringTodayList: React.FC = () => {
  const { data, isLoading } = useQuery<Array<Array<TVDetail>>, AxiosError>({
    queryKey: ['TVAiringTodayList'],
    queryFn: async () => {
      const info = await TvAiringTodayApi();
  ...
  return (
    <Container>
      <Row className='flex-column row-gap-5'>
        <Col className='text-white'>오늘 방영 프로그램</Col>
        <Col>
          <SubSlider data={data} />
        </Col>
      </Row>
...
export default TVAiringTodayList;
